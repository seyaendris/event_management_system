...
const EventForm = ({ userId, type, event, eventId }: EventFormProps) => {
    ...
    // Watch the isVip and price values
    ...
    async function onSubmit(values: z.infer<typeof eventFormSchema>) {
      ...
      if(type === 'Create') {
        try {
          ...
          if(newEvent) {
            form.reset();
            router.push(`/details/${newEvent._id}`)
          }
        } catch (error) {
          console.log(error);
        }
              ...
              <FormItem className="w-full">
                ...
                <div className="flex-center h-[54px] w-full overflow-hidden rounded-lg bg-grey-50 px-4 py-2">
                    <Image
                        ...
                        height={24}
                    />
                    <Input placeholder="If it is Online provide a URL" {...field} className="input-field" />
                </div>
                </FormControl>
                <FormMessage />
              </FormItem>
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <div className="flex items-center pl-4 mt-4">
                  <label htmlFor="isVip" className="whitespace-nowrap pr-3 leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70">
                      VIP Ticket Included?
                  </label>
                  <Checkbox
                      onCheckedChange={field.onChange}
